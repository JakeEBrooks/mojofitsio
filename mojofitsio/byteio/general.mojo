...
fn get_FITS_blocks(file: FileHandle, startblock: UInt64) raises -> Tensor[DType.int8]:
    """
...
fn get_FITS_blocks(file: FileHandle, startblock: UInt64, endblock: UInt64) raises -> Tensor[DType.int8]:
    """
    Return an integral number of FITS blocks in the range [*startblock*, *endblock*).
    """
    var bc = file.seek(config.fits_block_length*startblock)
    return file.read_bytes(config.fits_block_length*int((endblock - startblock)))
